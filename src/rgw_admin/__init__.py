...
class S3Auth(AuthBase):
    ...
    def __call__(self, request):
        ...
        request.headers["Authorization"] = "AWS {}:{}".format(
            self._hmac_auth.credentials.access_key, signature
        )
...
class AdminClient:
    def __init__(
        self, url, access_key, secret_key, *, verify_cert=True, redundant_url=None
    ):
        self._url = url
    ...
    def _request(self, method, path, schema=None, *, params=None, data=None):
        def _do_request(url):
            return getattr(self._session, method)(url, params=params, data=data)
    ...
    def delete_bucket(self, name, purge_data=False):
        return self._delete(
            "bucket", params={"bucket": name, "purge-objects": purge_data}
        )

    def delete_user(self, user_id, purge_data=True):
        return self._delete("user", params={"uid": user_id, "purge-data": purge_data})
    ...
    def update_user(self, user_id, *, display_name=None):
        ...
        return self._post("user", serialization.User, params=params)

    def create_user(self, user_id, display_name, *, user_caps={}):
        user_caps_str = ";".join(
            "{}={}".format(cap, ",".join(rights)) for cap, rights in user_caps.items()
        )
    ...
    def get_usage(self, user_id=None, start: int | None = None, end: int | None = None):
        ...
        return self._get(
            "usage",
            serialization.Usage,
            params={"uid": user_id, "start": start, "end": end, "show-summary": False},
        )
    ...
    def generate_new_secret_key(self, user_id, access_key):
        ...
        return self._put("user", serialization.KeyEntryList, params=params)
    ...
    def delete_key(self, user_id, access_key):
        ...
        dct = {"uid": user_id, "access-key": access_key}
        params = "key&" + urlencode(dct)
        return self._delete("user", params=params)

    def delete_usage(self, until: date, user_id=None):
        assert isinstance(until, date), until
        if user_id:
            return self._delete("usage", params={"uid": user_id, "end": str(until)})
        else:
            return self._delete("usage", params={"end": str(until), "remove-all": True})
